...
const generateAccessAndRefreshTokens = async ( userId ) => 
    ...
    try{
        ...
        throw new ApiError(500,"something went wrong while generating refresh and generate token")
    }
}
...
const loginUser = asyncHandler( async ( req,res) => {
    ...
    // username or email kis base  per login
    // find the user
    // password check
...
const getUserChannelProfile = asyncHandler(async (req,res) => {
    ...
    if(!username?.trim()){
        throw new ApiError(400,"Usernamme is missing")
    }
...
const getWatchHistory = asyncHandler(async (req,res) => {
    ...
    .json(
        new ApiResponse(
            200,
            user[0].watchHistory,
            "Watch history fetched successffuly"
        )
    )
})
